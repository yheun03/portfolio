<template>
  <div class="area-projects">
    ...
    >
      <SwiperSlide v-for="project in projects" :key="project.id" class="proj">
        <div class="content">
          ...
          <div class="proj-information">
            ...
            <button class="btn type-round bg-primary" @click="activeProject = project">상세보기</button>
          </div>
        </div>
      </SwiperSlide>
    </Swiper>
  ...
  <div v-if="activeProject" class="modal">
    <div class="modal-content active">
      <div class="modal-header">
        <h4>{{ activeProject.headline.replaceAll("<br>", " ") }}</h4>
        <ul>
          <li v-for="tag in activeProject.detailTags" :key="tag">{{ tag }}</li>
        </ul>
        <button class="btn type-round bg-black modal-close" aria-label="모달 닫기 버튼" @click="activeProject = null">닫기</button>
      </div>
      <div class="modal-body">
        <div class="wrap">
          <section v-for="section in activeProject.detailSections" :key="section.title">
            <h5>{{ section.title }}</h5>
