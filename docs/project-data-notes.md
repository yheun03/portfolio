...
| `stores/localeStore.ts`         | 언어 등 앱 환경 설정 상태                           |
| `stores/worksUiStore.ts`        | Works 필터·상세 모달 UI 상태                        |
| `stores/highlightsUiStore.ts`   | Highlights 탭 UI 상태                               |
| `composables/`, `plugins/`      | 화면 로직, Nuxt 플러그인                            |
| `app/`, `core/`                 | 라우터 옵션과 앱 설정성 TS 모듈                     |
...
| 파일                           | 이유                                                                                          |
| ------------------------------ | --------------------------------------------------------------------------------------------- |
| `core/config/home-sections.ts` | 홈 **섹션 키**·**레이어 그룹** — `HomeSectionKey` 타입과 `pages/index.vue` 매핑과 맞추기 쉬움 |
...
5. **홈 섹션 순서/그룹** → `core/config/home-sections.ts`

---

## 9. 관련 코드 진입점

- 작업 목록·필터·모달: `stores/worksUiStore.ts`, `components/home/PortfolioWorks.vue`
- 갤러리: `components/gallery/GalleryArchivePage.vue`, `GalleryDetailPage.vue` (공통 화면) · `composables/gallery/useGallery.ts` (아카이브·라우트 데이터) · `pages/projects/*`, `pages/personal/*` (URL 엔트리)
